defmodule OpenchatElixirWeb.E2E.TimelineApiTest do
  ...
  test "submit post and fetch user timeline", %{conn: conn} do
    conn = post(conn, "/users", %{ username: "shady90", password: "any", about: "any" })
    user_id = json_response(conn, 201)["id"]

    conn = post(conn, "/users/#{user_id}/timeline", %{ text: "First shady90 post." })
    ...
    conn = post(conn, "/users/#{user_id}/timeline", %{ text: "Second shady90 post." })
    %{ "postId" => secondPostId, "dateTime" => secondPostDateTime } = json_response(conn, 201)
